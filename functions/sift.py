...
def featureMatch(imagesPaths,sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
    ...
    # Calculate Euclidean distances between descriptors
    ...
    for i in range(len(descriptors1)):
        ...
        if distances[i,best_match] < 0.7 * distances[i,second_best_match]:
            matches.append((i, best_match))

    # Draw matched keypoints on the images
    resultImage = np.zeros((max(gray1.shape[0], gray2.shape[0]), gray1.shape[1]+gray2.shape[1], 3), dtype=np.uint8)
    resultImage[:gray1.shape[0], :gray1.shape[1], :] = image1
    resultImage[:gray2.shape[0], gray1.shape[1]:, :] = image2
    for match in matches:
        pt1 = tuple(map(int, keyPoints1[match[0]].pt))
        pt2 = tuple(map(int, keyPoints2[match[1]].pt))
        pt2 = (pt2[0] + gray1.shape[1], pt2[1])
        color = tuple(np.random.randint(0, 255, 3).tolist())
        cv2.line(resultImage, pt1, pt2, color, thickness=5)

    cv2.imwrite("siftOutputImage.png",resultImage)
